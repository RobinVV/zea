...
def compute_normals_xz(probe_geometry):
    ...
    N = probe_geometry.shape[0]
    central_diff = probe_geometry[2:] - probe_geometry[:-2]
    first_diff = probe_geometry[1:2] - probe_geometry[0:1]
    last_diff = probe_geometry[-1:] - probe_geometry[-2:-1]

    tangent = ops.concatenate([first_diff, central_diff, last_diff], axis=0)

    # Zero out y-component
    tangent_xz = ops.stack([tangent[:, 0], ops.zeros(N), tangent[:, 2]], axis=1)
