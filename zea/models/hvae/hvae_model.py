...
from zea.models.hvae.hvae_utils import SoftPlus, gaussian_analytical_kl, GradientNorms
...
class VAE(Model):
    ...
    def call(self, x):
        # This gives [vq] per stage for normal vdvae
    ...
    def sample_from_mol(self, logits, t=1.0):
        ...
        gumbel_noise = -ops.log(
            -ops.log(random.uniform((B, H, W, self.params.num_output_mixtures), minval=1e-5, maxval=1. - 1e-5))
        )  # B, H, W, M
        amax = ops.argmax(logit_probs / t + gumbel_noise, axis=-1)  # B, H, W
        lambda_ = ops.cast(ops.one_hot(amax, self.params.num_output_mixtures, axis=-1), "float32")  # B, H, W, M
        lambda_ = ops.expand_dims(lambda_, axis=3)  # B, H, W, 1, M
        
        
        means = ops.sum(l[:, :, :, :, : self.params.num_output_mixtures] * lambda_, axis=-1)  # B, H, W, C
        
        log_scales = self.min_mol_logscale + self.softplus(l[:, :, :, :, self.params.num_output_mixtures : 2 * self.params.num_output_mixtures] - self.min_mol_logscale)  # B, H, W, C
        log_scales = ops.sum(log_scales * lambda_, axis=-1)  # B, H, W, C
        
        coeffs = ops.sum(ops.tanh(
            l[:, :, :, :, 2 * self.params.num_output_mixtures : 3 * self.params.num_output_mixtures]
            ) * lambda_, axis=-1)  # B, H, W, C


        # Sample from logistic
        u = random.uniform((B, H, W, self.params.data_width), minval=1e-5, maxval=1. - 1e-5)
...
class Decoder(layers.Layer):
    def __init__(self, params):
        ...
        self.last_conv = layers.Conv2D(
            params.channels_out, kernel_size=params.kernelsizes[str(params.dec_input_size[-1])], padding="same"
        )
...
class Encoder(layers.Layer):
    def __init__(self, params):
        ...
        self.first_conv = layers.Conv2D(
            params.enc_in_width[0], kernel_size=params.kernelsizes[str(params.enc_input_size[0])], padding="same"
        )
...
class DecBlock(layers.Layer):
    def __init__(
        ...
        if z_out[0] > input_size:
            self.z_out_up = layers.UpSampling2D(size=z_out[0]//input_size,
    ...
    def sample(self, x, act):
        ...
        if self.use_depthwise_attention:
            ...
            q_out = self.q(ops.concatenate([x, vq[:, :, :, :, -1] + self.gamma_q * attn_q], axis=-1))
        else:
...
class Block(layers.Layer):
    def __init__(
        ...
        # Groupnorm layers
        self.gn1 = (
            layers.GroupNormalization(groups=in_width//8
                                      )
            if bn
        ...
        if depthwise:
            ...
            self.c2 = (
                layers.Conv2D(middle_width, kernel_size=kernelsize, padding="same")
            )
            self.c3 = (
                layers.Conv2D(middle_width, kernel_size=kernelsize, padding="same")
            )
        ...
        if self.use_attention:
            self.attention1 = SpatialSelfAttention(middle_width, attention_width)
            self.attention2 = SpatialSelfAttention(middle_width, attention_width)
