...
class Parameters:
    ...
    def verify_parameters(self):
        ...
        # If we are using an orthogonal sylvester flow in a stage, it cannot have more orthogonal vectors than z_dimensions
...
class DiscMixLogistic():
    def __init__(self, num_bits, num_mixtures, num_channels):
        ...
        self.cone_loss_mask = cone_loss_mask(batch_size=1, r_max=256, angle=0.7854, shape=(256, 256))
    ...
    def call(self, targets, logits, mask=None):
        ...
        targets = ops.cast(
            ops.expand_dims(targets, -1), dtype=self.dt
        )  # B, H, W, C, 1
        ...
        log_scales = self.min_mol_logscale + self.softplus(l[:, :, :, :, self.num_mixtures : 2 * self.num_mixtures] - self.min_mol_logscale)
        ...
        if C == 3:
            ...
            mean2 = (
                ops.add(model_means[:, :, :, 1:2, :],
                ops.multiply(model_coeffs[:, :, :, 0:1, :], targets[:, :, :, 0:1, :]))
            )  # B, H, W, 1, M
            mean3 = (
                ops.add(ops.add(model_means[:, :, :, 2:3, :],
        ...
        if mask is not None:
            ...
            scale = ops.clip(256*256 / num_pixels, 1.0, 256) # scale is 1 for fully sampled, 256 for 1 line.
            loss *= scale
        return loss
